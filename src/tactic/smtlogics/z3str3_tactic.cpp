...
tactic * mk_z3str3_tactic(ast_manager & m, params_ref const & p) {
    ...
    if (m_smt_params.m_StrTactic == symbol("all")) {
        ...
        tactic * st = using_params(and_then(and_then(mk_rewriter_tactic(m, p), mk_ext_str_tactic(m, p)), z3str3_2), p);
        return st;
        ...
        tactic * st = using_params(and_then(mk_rewriter_tactic(m, p), or_else(z3str3_1, z3str3_2)), p);
        return st;
