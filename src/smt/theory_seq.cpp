...
bool theory_seq::propagate_max_length(expr* l, expr* r, dependency* deps) {
    ...
    rational hi;
    if (m_sk.is_tail(l, s, idx) && has_length(s) && m_util.str.is_empty(r) && !upper_bound(s, hi)) {
...
bool theory_seq::get_length(expr* e, expr_ref& len, literal_vector& lits) {
    ...
    else if (m_sk.is_tail_match(e, s, l)) {
...
expr_ref theory_seq::elim_skolem(expr* e) {
    ...
    while (!todo.empty()) {
        ...
        if (m_sk.is_tail_match(a, x, y) && cache.contains(x) && cache.contains(y)) {
            x = cache[x];
            y = cache[y];
